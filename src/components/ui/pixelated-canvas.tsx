...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  const [colors, setColors] = useState({ tint: "#000" });

  useEffect(() => {
    setColors({
      tint: getCSSVariable("--foreground") || "#000",
    });
  }, []);
  ...
  React.useEffect(() => {
    ...
    const img: HTMLImageElement & { _cleanup?: () => void } = new Image();
